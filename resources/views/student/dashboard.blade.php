...
<div id="payment-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm hidden">
    <div class="glass-card w-full max-w-md overflow-hidden bg-white/95 shadow-2xl relative">
        ...
        {{-- Header with Close button in flex container to avoid absolute alignment overlapping --}}
        <div class="px-6 pt-6 pb-4 border-b border-gray-100 flex items-start justify-between">
            <div>
                <h3 class="text-base font-bold text-gray-800" id="modal-title">Bayar Tagihan</h3>
                <p class="text-xs font-semibold text-emerald-700 mt-1" id="modal-amount-label">Nominal: Rp 0</p>
            </div>
            <button onclick="closePaymentModal()" class="text-gray-400 hover:text-gray-600 transition-colors p-1.5 rounded-lg hover:bg-gray-100 cursor-pointer">
        ...
        {{-- Step Navigation Indicator (Non-interactive tabs for clear guided flow) --}}
        <div class="px-6 py-3 bg-gray-50 border-b border-gray-100">
            <div class="flex items-center justify-center gap-2 max-w-xs mx-auto">
                <!-- Step 1 Dot -->
                <div id="step-dot-wa" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">
                    <span id="step-num-wa" class="w-5 h-5 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-2xs">1</span>
                    <span>Transfer</span>
                </div>
                <!-- Line -->
                <div class="flex-1 h-0.5 bg-gray-200"></div>
                <!-- Step 2 Dot -->
                <div id="step-dot-proof" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-500">
                    <span id="step-num-proof" class="w-5 h-5 rounded-full bg-gray-300 text-white flex items-center justify-center font-bold text-2xs">2</span>
                    <span>Bukti</span>
                </div>
            </div>
        </div>
        ...
        <div class="p-6 space-y-4">
            {{-- Step 1 Content: Account Information --}}
            <div id="tab-content-wa" class="space-y-4">
                
                {{-- Info Rekening Sekolah --}}
                <div class="p-4 rounded-xl bg-emerald-50/50 border border-emerald-100/80 space-y-3">
                    <h4 class="text-xxs font-bold text-emerald-800 uppercase tracking-wider">Rekening Transfer Resmi</h4>
                    
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="text-xs text-gray-400">Bank</span>
                            <span class="text-xs font-bold text-gray-800">BANK BRI</span>
                        </div>
                        
                        <div class="flex items-center justify-between">
                            <span class="text-xs text-gray-400">Nomor Rekening</span>
                            <div class="flex items-center gap-1.5">
                                <span class="text-sm font-mono font-bold text-gray-800">657401013488535</span>
                                <button type="button" onclick="copyToClipboard('657401013488535', this)" 
                                    class="px-2 py-0.5 rounded border border-emerald-200 bg-white hover:bg-emerald-50 text-emerald-700 text-xxs font-medium flex items-center gap-1 transition-all cursor-pointer">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-xs text-gray-400">Atas Nama (A.N.)</span>
                            <span class="text-xs font-semibold text-gray-800">Uswatun Hasanah</span>
                        </div>
                    </div>
                </div>

                {{-- WhatsApp Optional Notification dropdown --}}
                <div class="border border-gray-100 rounded-xl overflow-hidden bg-white/50 shadow-sm">
                    <button type="button" onclick="toggleWaSection()" 
                        class="w-full px-4 py-2.5 bg-gray-50/50 hover:bg-gray-50 text-left text-xs font-semibold text-gray-600 flex items-center justify-between transition-colors cursor-pointer">
                        <span class="flex items-center gap-2">
                            ...
                            Kirim Rincian ke WhatsApp (Opsional)
                        </span>
                        <svg id="wa-toggle-icon" class="w-4 h-4 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <div id="wa-section-content" class="p-4 space-y-2 hidden border-t border-gray-100 bg-white/70">
                        <p class="text-xxs text-gray-500 leading-relaxed">
                            Kirim detail rekening dan nominal pembayaran ini secara otomatis ke WhatsApp Anda:
                        </p>
                        <form id="pay-wa-form" method="POST" class="flex gap-2">
                            @csrf
                            <div class="flex-1">
                                <input type="text" id="whatsapp_number" name="whatsapp_number" required placeholder="Contoh: 081234567890"
                                    class="w-full px-3 py-2 rounded-xl border border-gray-200 bg-white text-xs focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all">
                            </div>
                            <button type="submit" onclick="autoTransitionToUpload()"
                                class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer whitespace-nowrap shadow-sm">
                                Kirim
                            </button>
                        </form>
                    </div>
                ...
                <div class="pt-2">
                    <button type="button" onclick="switchTab('proof')"
                        ...
                        Saya Sudah Transfer, Unggah Bukti →
                    </button>
                </div>
            </div>

            {{-- Step 2 Content: Proof upload --}}
            <div id="tab-content-proof" class="space-y-4 hidden">
                ...
                <form id="upload-proof-form" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div class="space-y-2">
                        <label for="payment_proof" class="block text-xxs font-bold text-gray-650 mb-1 uppercase">Pilih File Bukti</label>
                        
                        {{-- Custom Premium Upload Dropzone look --}}
                        <div onclick="document.getElementById('payment_proof').click()" 
                            class="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center cursor-pointer hover:border-primary-400 hover:bg-primary-50/10 transition-all flex flex-col items-center justify-center gap-2 group">
                            <input type="file" id="payment_proof" name="payment_proof" accept="image/*" required class="hidden" onchange="handleFileSelected(this)">
                            
                            <div class="w-12 h-12 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors">
                            ...
                            <div>
                                <p id="upload-zone-text" class="text-xs font-semibold text-gray-700 group-hover:text-primary-750 transition-colors">Pilih file gambar bukti transfer</p>
                                <p class="text-xxs text-gray-400 mt-1">Hanya gambar (JPG, JPEG, PNG). Maksimal 2 MB.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 pt-2">
                        <button type="button" onclick="switchTab('wa')"
                            class="flex-1 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-600 font-semibold text-xs transition-all cursor-pointer text-center">
                            ← Kembali
                        </button>
                        <button type="submit"
                            class="flex-2 py-2.5 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-semibold text-xs shadow-md shadow-primary-600/10 transition-all cursor-pointer text-center">
...
function openPaymentModal(billId, billName, amount, waNumber) {
    ...
    // Collapse WhatsApp section by default
    const waContent = document.getElementById('wa-section-content');
    const waIcon = document.getElementById('wa-toggle-icon');
    waContent.classList.add('hidden');
    waIcon.classList.remove('rotate-180');
}
...
function switchTab(tab) {
    const stepWa = document.getElementById('step-dot-wa');
    const stepProof = document.getElementById('step-dot-proof');
    const contentWa = document.getElementById('tab-content-wa');
    const contentProof = document.getElementById('tab-content-proof');

    if (tab === 'wa') {
        // Step 1 Active
        stepWa.className = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700";
        stepWa.innerHTML = `
            <span class="w-5 h-5 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-2xs">1</span>
            <span>Transfer</span>
        `;
        
        stepProof.className = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-500";
        stepProof.innerHTML = `
            <span class="w-5 h-5 rounded-full bg-gray-300 text-white flex items-center justify-center font-bold text-2xs">2</span>
            <span>Bukti</span>
        `;
        ...
        // Step 2 Active
        stepWa.className = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-600";
        stepWa.innerHTML = `
            <span class="w-5 h-5 rounded-full bg-emerald-100 text-emerald-650 flex items-center justify-center font-bold text-2xs">
        ...
        stepProof.className = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-primary-100 text-primary-700";
        stepProof.innerHTML = `
            <span class="w-5 h-5 rounded-full bg-primary-600 text-white flex items-center justify-center font-bold text-2xs">2</span>
            <span>Bukti</span>
        `;
...
function copyToClipboard(text, button) {
    navigator.clipboard.writeText(text).then(() => {
        ...
        button.innerHTML = `
            ...
            Tersalin!
        `;
        button.classList.remove('border-emerald-200', 'text-emerald-700');
        ...
        setTimeout(() => {
            ...
            button.classList.add('border-emerald-200', 'text-emerald-700');
        }, 2000);
    }).catch(err => {
...
function toggleWaSection() {
    const waContent = document.getElementById('wa-section-content');
    const waIcon = document.getElementById('wa-toggle-icon');
    if (waContent.classList.contains('hidden')) {
        waContent.classList.remove('hidden');
        waIcon.classList.add('rotate-180');
    } else {
        waContent.classList.add('hidden');
        waIcon.classList.remove('rotate-180');
    }
...
function handleFileSelected(input) {
    const textElement = document.getElementById('upload-zone-text');
    if (input.files && input.files[0]) {
        const file = input.files[0];
        textElement.textContent = "Terpilih: " + file.name + " (" + (file.size / 1024 / 1024).toFixed(2) + " MB)";
        textElement.classList.remove('text-gray-700');
        textElement.classList.add('text-primary-700');
    } else {
        textElement.textContent = "Pilih file gambar bukti transfer";
        textElement.classList.remove('text-primary-700');
